...
Simple drag to go back functionality.  Just like pull to refresh, but for back.

## Installation

`npm install drag-to-go-back --save`

## Usage

You'll need to import the package wherever you use it:

```js
import { dragToGoBack } from 'drag-to-go-back';
```

You can then invoke it on any page using the `init` function:

```js
dragToGoBack.init();
```

For example, if you're using Blaze templates in Meteor:
...
You'll then just need to add the `drag-to-go-back` class to which ever div you want to be draggable.  This class name is customizable via the `configure` function.

## CSS

This package requires you to provide it some CSS.  It is not included automatcially so that you can easily customize it as you see fit.

Start with this snippet and go from there:
...
.drag-to-go-back-indicator-container {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
.drag-to-go-back-indicator-icon {
  ...
  width: 50px;
  height: 50px;
  background-image: url('data:image/png;base64,');
...
>If you change the CSS class names, be sure to update them in both the CSS and the configuration

## Configuration (optional)

A configuration function is provided.  The default values are:
...
**threshold**: The percentage that you need to drag past in order to trigger an event.

**containerClass**: The name of the class you'll add to the div you want to be draggable.

**handleClass**: The class of the automatically added handle div.

**indicatorContainerClass**: The class of the automatically added indicator div.

**indicatorBackgroundClass**: The class of the automatically added indicator background div.

**indicatorIconClass**: The class of the automatically added indicator icon div.

**onComplete**: The callback that happens after the drag to go back completes

## Using with a transition

To use with a transition animation you'll need to configure the `onComplete` callback.  For example, if you're using this with the `native-transition` and `tab-history` packages in Meteor:
